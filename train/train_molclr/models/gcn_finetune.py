...
class GCN(nn.Module):
    ...
    #     node_embeddings = []  # Store node embeddings per layer
    #     for layer in range(self.num_layer):
    #         h = self.gnns[layer](h, edge_index, edge_attr)
    #         h = self.batch_norms[layer](h)
    #         if layer == self.num_layer - 1:
    #             node_embeddings.append(h.detach())  # Save last layer embeddings
    #             h = F.dropout(h, self.drop_ratio, training=self.training)
    #         else:
    #             h = F.dropout(F.relu(h), self.drop_ratio, training=self.training)
    ...
    #     # Compute interaction heatmap (cosine similarity)
    #     last_embeddings = node_embeddings[-1]
    #     # heatmap = torch.mm(last_embeddings, last_embeddings.T)  # Dot product
    #     heatmap = F.cosine_similarity(last_embeddings.unsqueeze(1), last_embeddings.unsqueeze(0), dim=2)  # Cosine similarity

    #     return h, self.pred_head(h), heatmap
