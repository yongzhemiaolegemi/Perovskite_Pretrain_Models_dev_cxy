...
class MolTestDatasetWrapper(object):
    ...
    def __init__(self, 
        ...
        data_path, target, task, splitting,split_id,
    ):
    ...
    def get_data_loaders_p(self):
        # print(self.data_path)
        # exit()
        train_dataset = MolTestDataset(self.data_path, target=self.target, task=self.task)
        test_dataset  = MolTestDataset(self.data_path.replace('train','test_cleaned'), target=self.target, task=self.task)
        
        train_loader, valid_loader, test_loader = self.get_train_validation_data_loaders_p(train_dataset, self.split_id)
        # train_loader, valid_loader, _ = self.get_train_validation_data_loaders_p(train_dataset)
    ...
    def get_train_validation_data_loaders(self, train_dataset):
        ...
        return train_loader, valid_loader, test_loader
    def get_train_validation_data_loaders_p(self, train_dataset, split_id):
        if self.splitting == 'random':
            # obtain training indices that will be used for validation
            dir = '/data/yhm/cxy_molclr/MolCLR/data/perovskite-resplit/splits.json'
            import json
            with open(dir, 'r') as f:
                data = json.load(f)
            train_idx = data['train_indices'][split_id]
            valid_idx = data['valid_indices'][split_id]


            print("-- Using Fixed splitting from splits.json--")

            print(" Data Loaded from JSON file")
            print("train_size: ", len(train_idx))
            print("valid_size: ", len(valid_idx))
    ...
    def get_test_data_loaders(self, train_dataset):
        if self.splitting == 'random':
            # obtain training indices that will be used for validation
            self.test_size = 0.0
            num_train = len(train_dataset)
